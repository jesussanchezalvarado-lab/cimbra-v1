...
(() => {
  ...
  function collectProps(node, kind, host) {
    ...
    for (const { name, value } of [...node.attributes]) {
      ...
      if (key.startsWith("sc-style-")) {
        const styleKey = key.slice("sc-style-".length);
        const normalized = styleKey === "bg" ? "background" : kebabToCamel(styleKey);
        styleGetters.push([normalized, compileAttr(value)]);
        continue;
      }
      if (key === "sc-onclick") {
        propGetters.push(["onClick", compileAttr(value)]);
        continue;
      }
      if (key === "sc-onchange") {
        propGetters.push(["onChange", compileAttr(value)]);
        continue;
      }
      if (key === "sc-oninput") {
        propGetters.push(["onInput", compileAttr(value)]);
        continue;
      }
      if (kind !== "dom") {
    ...
    return { propGetters, pseudoClasses, hintSize, styleGetters };
  }
  ...
  function walkElement(el, host) {
    ...
    const { propGetters, pseudoClasses, styleGetters } = collectProps(el, "dom", host);
    const kids = walkChildren(el, host);
    return (vals, ctx, key) => {
      const props = {
        key: key + keySuffix,
      ...
      let styleObj = props.style && typeof props.style === "object" ? { ...props.style } : null;
      for (const [k, g] of styleGetters) {
        const v = g(vals);
        if (v !== void 0) {
          styleObj = styleObj || {};
          styleObj[k] = v;
  ...
  // src/cdn.ts
  var REACT_URL = "../comun/lib/react.production.min.js";
  var REACT_SRI = "";
  var REACT_DOM_URL = "../comun/lib/react-dom.production.min.js";
  var REACT_DOM_SRI = "";
